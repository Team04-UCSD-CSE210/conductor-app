...
describe('AuditService', () => {
  ...
  // Helper function to ensure test user exists and return its ID
  async function ensureTestUser() {
    // Always look up by email to get the current ID (handles case where user was deleted/recreated)
    let user = await pool.query('SELECT id FROM users WHERE email = $1 AND deleted_at IS NULL', [TEST_USER_EMAIL]);
    if (user.rows.length === 0) {
      // User doesn't exist - create it (ON CONFLICT will restore if soft-deleted)
